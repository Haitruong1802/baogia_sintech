...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const item of items) {
      ...
      ws.getCell(`B${startRow}`).value = item.name;
      ws.getCell(`C${startRow}`).value = item.brand;
      ws.getCell(`D${startRow}`).value = item.qty;
      ws.getCell(`E${startRow}`).value = item.price;
      // ws.getCell(`F${startRow}`).value = item.total;
      ws.getCell(`G${startRow}`).value = item.warranty;
      startRow++; stt++;
    }
